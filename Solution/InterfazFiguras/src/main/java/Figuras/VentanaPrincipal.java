...
public class VentanaPrincipal extends JFrame implements ActionListener {
    ...
    public void actionPerformed(ActionEvent evento) {
        if (evento.getSource() == esfera) { // Si se pulsa el botón esfera
            VentanaEsfera ventanaEsfera = new VentanaEsfera(); /* Crea la
                                                                  ventana de la esfera */
            ventanaEsfera.setVisible(true); /* Establece que se visualice la ventana
                                               de la esfera */
        }
        if (evento.getSource() == cilindro) { /* Si se pulsa el botón
                                               cilindro */
            VentanaCilindro ventanaCilindro = new VentanaCilindro(); /* Crea la
                                                                      ventana del cilindro */
            ventanaCilindro.setVisible(true); /* Establece que se visualice la
                                                 ventana del cilindro */
        }
        if (evento.getSource() == pirámide) { /* Si se pulsa el botón
                                                 pirámide */
            VentanaPiramide ventanaPirámide = new VentanaPiramide(); /* Crea
                                                                       la ventana de la pirámide */
            ventanaPirámide.setVisible(true); /* Establece que se visualice la
